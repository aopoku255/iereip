...
import { getRejectedLoanApplications } from "../../slices/loans/thunk";

const RejectedApplications = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { rejectedLoans, rejectedLoansLoading, rejectedLoansError } = useSelector((state) => state.Loans || {});

  useEffect(() => {
    dispatch(getRejectedLoanApplications());
  }, [dispatch]);
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Rejected Applications" pageTitle="Loan Application" />
          <Row>
            <Col xl={12}>
              <Card>
                <CardHeader className="align-items-center d-flex">
                  <h4 className="card-title mb-0">Rejected Loan Applications</h4>
                </CardHeader>
                <CardBody>
                  {rejectedLoansLoading ? (
                    <div className="text-center py-5">
                      <Spinner color="primary" />
                    </div>
                  ) : rejectedLoansError ? (
                    <div className="text-danger py-4">{rejectedLoansError}</div>
                  ) : (
                    <TableContainer
                      columns={columns}
                      data={Array.isArray(rejectedLoans) ? rejectedLoans : []}
                      isGlobalFilter
                      topFiltersOnRight
                      isGlobalSearch
                      hideHeaderFilters
