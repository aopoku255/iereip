    ...
    const getPageTitle = (pathname) => {
        const findMenuItem = (items) => {
            for (const item of items) {
                ...
                if (item.subItems) {
                    const subItemMatch = findMenuItem(item.subItems);
                    if (subItemMatch) return subItemMatch;
                }
                if (item.childItems) {
                    const childItemMatch = findMenuItem(item.childItems);
                    if (childItemMatch) return childItemMatch;
                }
            }
            return null;
        };

        const matchedItem = findMenuItem(navData);
        if (matchedItem?.documentTitle) return matchedItem.documentTitle;
        if (matchedItem?.label) return `${matchedItem.label} | IEREIP Enterprice`;
 ...
 */
    ...
    // User selector
    const selectPermissionsState = (state) => state.Login?.permissions || [];
    const selectUserState = (state) => state.Login?.user || state.Profile?.user;
    const selectUserProperties = createSelector(
        [selectUserState, selectPermissionsState],
        (user, permissions) => ({
    ...
    const { permissions } = useSelector(selectUserProperties);

    // Filter menu items based on user permissions
    const filterMenuByPermissions = (items, userPermissions) => {
        if (!userPermissions || userPermissions.length === 0) return items; // Show all if no permissions defined
        return items.map(item => {
            if (item.isHeader) return item;
            if (item.permissions && !item.permissions.some(permission => userPermissions.includes(permission))) return null;
            if (item.subItems) {
                const filteredSubItems = item.subItems.filter(subItem => 
                    !subItem.permissions || subItem.permissions.some(permission => userPermissions.includes(permission))
                );
    ...
    const filteredNavData = filterMenuByPermissions(navData, permissions);
    ...
    useEffect(() => {
        const pageTitle = getPageTitle(props.router.location.pathname);
        if (pageTitle) {
    ...
    useEffect(() => {
        ...
        const initMenu = () => {
            const pathName = import.meta.env.BASE_URL + props.router.location.pathname;
